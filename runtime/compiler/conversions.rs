...
pub(crate) fn wasmparser_to_llvm_numtype(ctx: &LLContext, ty: &wasmparser::ValType) -> Box<dyn LLNumType> {
    match ty {
...
pub(crate) fn wasmparser_to_llvm_functype(ctx: &LLContext, ty: &wasmparser::FuncType) -> LLFunctionType {
    let params = ty
        .params
        .iter()
