        ...
        match &value_stack[..] {
            ...
            result_values => {
                ...
                for (index, value) in result_values.iter().enumerate() {
                    ...
                    let field_index = Box::new(LLIntType::constant(&llvm_context.i32_type(), index as u64, false));

                    let llvm_gep = llvm_builder.build_gep(&llvm_alloca, &[zero_index, field_index], "gep")?;
...
impl<'a> Generator for FunctionBodyGenerator<'a> {
    ...
    fn generate(&mut self) -> Result<()> {
        ...
        let mut llvm_func = llvm_module.add_function(&format!("func_{}", self.body_index), llvm_func_type)?;
        ...
        for (index, ty) in func_type.params.iter().enumerate() {
            ...
            let llvm_local = llvm_builder.build_alloca(llvm_local_ty.up(), &format!("param_{index}"))?;
        ...
        for local in locals_reader.into_iter() {
            ...
            for index in 0..count {
                ...
                let llvm_local = llvm_builder.build_alloca(llvm_local_ty.up(), &format!("local_{index}"))?;
        ...
        if !value_stack.is_empty() {
            Self::generate_return(llvm_context, &mut llvm_builder, &mut value_stack, &func_type.results)?;
        }
