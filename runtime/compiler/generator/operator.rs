...
use llvm::{
    ...
    intrinsics,
    module::LLModule,
    types::{LLFloatType, LLIntType, LLNumType},
    values::{LLAlloca, LLFloatPredicate, LLFunction, LLIntPredicate, LLValue},
};
...
pub(crate) enum Control {
    ...
    ///    └──────────┘
    /// ```
    If { r#else: LLBasicBlock, end: LLBasicBlock },
    /// ```text
    /// ┌─────────┐
    /// │   Loop  ◄──┐
    /// └────┬────┘  │
    ...
    /// └─────────┘
    /// ```
    Loop { begin: LLBasicBlock, end: LLBasicBlock },
    /// ```text
    /// ┌─────────┐
    /// │  Block  │
    /// └────┬────┘
    ...
    fn generate(&mut self) -> Result<()> {
        ...
        match self.operator {
            ...
            Operator::Nop => {
                ...
                self.llvm_builder.build_int_add(llvm_zero, llvm_zero, "nop")?;
            }
            Operator::Block { .. } => {
                let llvm_begin_bb = self
                    .llvm_func
                    .create_and_append_basic_block(&format!("block_begin.{}", block_count), self.llvm_context)?;

                let llvm_end_bb = LLBasicBlock::new(&format!("block_end.{}", block_count), self.llvm_context)?;
            ...
            Operator::Loop { .. } => {
                let llvm_begin_bb = self
                    .llvm_func
                    .create_and_append_basic_block(&format!("loop_begin.{}", block_count), self.llvm_context)?;

                let llvm_end_bb = LLBasicBlock::new(&format!("loop_end.{}", block_count), self.llvm_context)?;
            ...
            Operator::If { .. } => {
                let llvm_then_bb = self
                    .llvm_func
                    .create_and_append_basic_block(&format!("if_then.{}", block_count), self.llvm_context)?;

                let llvm_else_bb = self
                    .llvm_func
                    .create_and_append_basic_block(&format!("if_else.{}", block_count), self.llvm_context)?;

                let llvm_end_bb = LLBasicBlock::new(&format!("if_end.{}", block_count), self.llvm_context)?;
            ...
            Operator::I32Eqz | Operator::I64Eqz => {
                ...
                let llvm_result =
                    self.llvm_builder
                        .build_int_cmp(LLIntPredicate::EQ, operand.as_ref(), &llvm_const, "icmp_eqz")?;
            ...
            Operator::I32Add | Operator::I64Add => {
                ...
                let llvm_result = self.llvm_builder.build_int_add(lhs.as_ref(), rhs.as_ref(), "add")?;
            ...
            Operator::I32Sub | Operator::I64Sub => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_sub(lhs.as_ref(), rhs.as_ref(), "sub")?;
            ...
            Operator::I32Mul | Operator::I64Mul => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_mul(lhs.as_ref(), rhs.as_ref(), "mul")?;
            ...
            Operator::I32DivS | Operator::I64DivS => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_sdiv(lhs.as_ref(), rhs.as_ref(), "div_s")?;
            ...
            Operator::I32DivU | Operator::I64DivU => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_udiv(lhs.as_ref(), rhs.as_ref(), "div_u")?;
            ...
            Operator::I32RemS | Operator::I64RemS => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_srem(lhs.as_ref(), rhs.as_ref(), "rem_s")?;
            ...
            Operator::I32RemU | Operator::I64RemU => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_urem(lhs.as_ref(), rhs.as_ref(), "rem_u")?;
            ...
            Operator::I32And | Operator::I64And => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_and(lhs.as_ref(), rhs.as_ref(), "and")?;
            ...
            Operator::I32Or | Operator::I64Or => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_or(lhs.as_ref(), rhs.as_ref(), "or")?;
            ...
            Operator::I32Xor | Operator::I64Xor => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_xor(lhs.as_ref(), rhs.as_ref(), "xor")?;
            ...
            Operator::I32Shl | Operator::I64Shl => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_shl(lhs.as_ref(), rhs.as_ref(), "shl")?;
            ...
            Operator::I32ShrS | Operator::I64ShrS => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_ashr(lhs.as_ref(), rhs.as_ref(), "shr_s")?;
            ...
            Operator::I32ShrU | Operator::I64ShrU => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_int_lshr(lhs.as_ref(), rhs.as_ref(), "shr_u")?;
            ...
            Operator::F32Add | Operator::F64Add => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_float_add(lhs.as_ref(), rhs.as_ref(), "add")?;
            ...
            Operator::F32Sub | Operator::F64Sub => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_float_sub(lhs.as_ref(), rhs.as_ref(), "sub")?;
            ...
            Operator::F32Mul | Operator::F64Mul => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_float_mul(lhs.as_ref(), rhs.as_ref(), "mul")?;
            ...
            Operator::F32Div | Operator::F64Div => {
                let rhs = self.value_stack.pop().unwrap();
                let lhs = self.value_stack.pop().unwrap();
                let llvm_result = self.llvm_builder.build_float_div(lhs.as_ref(), rhs.as_ref(), "div")?;
