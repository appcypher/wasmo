...
use llvm_sys::{
    core::{
        LLVMConstInt, LLVMConstNull, LLVMConstReal, LLVMDoubleTypeInContext, LLVMFloatTypeInContext, LLVMFunctionType,
        LLVMGetTypeKind, LLVMInt128TypeInContext, LLVMInt32TypeInContext, LLVMInt64TypeInContext, LLVMStructType,
...
pub trait LLNumType: LLValueType + LLResultType + Upcast<dyn LLResultType> + Upcast<dyn LLValueType> {
    /// Returns the underlying LLVMTypeRef of this value.
    ///
    /// # Safety
    /// - Unsafe because it exposes a raw pointer gotten from LLVM ffi.
...
impl LLStructType {
    ...
    pub fn new(types: &[Box<dyn LLNumType>], is_packed: bool) -> Self {
        ...
        Self(unsafe { LLVMStructType(types.as_ptr() as *mut LLVMTypeRef, types.len() as u32, is_packed as i32) })
    }
...
impl LLFunctionType {
    ...
    pub fn new(params: &[Box<dyn LLNumType>], result: &dyn LLResultType, is_varargs: bool) -> Self {
        let params = params.iter().map(|p| unsafe { p.num_ref() }).collect::<Vec<_>>();
