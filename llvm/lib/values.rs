...
pub trait LLValue: DynClone {
    ...
    fn is_pointer_type(&self) -> bool {
        unsafe { LLVMTypeKind::LLVMPointerTypeKind == LLVMGetTypeKind(LLVMTypeOf(self.value_ref())) }
    }
}
...
impl LLFunction {
    ...
    pub(super) fn new(name: &str, module: &mut LLModule, function_type: &LLFunctionType) -> Result<Self> {
        Ok(Self(unsafe {
    ...
    pub fn create_and_append_basic_block(&self, name: &str, context: &LLContext) -> Result<LLBasicBlock> {
